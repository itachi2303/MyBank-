...
console.log("Showing current status of your account...")
 ...
 class showstatus {
    bank_name: string = "mybank";
    user_name:string = name.username;
    acc_id: number = ask.id;
    acc_balance:number = balance;
    constructor(bank_name:string,user_name:string,acc_id:number,acc_balance:number)
    {
...
const show:showstatus = new showstatus("mybank",name.username,333,balance)
obj.push(show)
console.table(obj);
...
if(options.opt === "Deposite")
    {
        ...
        if(askme.askmee <= 0){console.log("please enter a valid amount")}
        else{
...
if(askme2.askmee3 <= 0){console.log("please enter a valid amount")}
else if(askme2.askmee3 > balance){console.log("we are sorry your there are insufficant funds in your ascount for this withdrawl")}
        else{
            balance = balance - askme2.askmee3;
            console.log("Your balance is =>:",balance);
    }
    }
    else if(options.opt ==="exit") {console.log("Have a nice day (^-^)");
    process.exit(0)}
...
if(furopt.futher_options == "[Updated status]")
    {
        const showtable:showstatus = new showstatus('mybank',name.username,333,balance)
...
else{console.log("wrong password please try again :(")}
